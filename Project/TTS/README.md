...
瀏覽器會開啟 `http://127.0.0.1:8000`。輸入英文後，可以單獨生成某一階段，也可以一次生成全部七個階段並直接播放或下載。生成時會顯示各階段的處理步驟、等待秒數、估計百分比與整體進度；完成的階段也會出現在頁面上方的快速播放列。

目前限制：只接受英文 ASCII 文字，最多 300 個字元。Stage 5 的 Tacotron 2 使用 GPU，WaveRNN vocoder 則使用 CPU 逐點生成，通常需要等待約 1–2 分鐘；在目前 Windows 環境中，WaveRNN 放在 GPU 反而更慢。
